...
test("should set a new task for a user", async () => {
  ...
  // Mocking the create method to return the created task
...
test("should return a 401 error if user is not found", async () => {
  ...
  // Mocking the findById method to return null for the user, indicating that the user does not exist
...
test("should return a 401 error if user is not authorized to update the task", async () => {
  ...
  const taskToUpdate = {
    _id: taskId,
    text: "Original Task",
    user: "user-id-1", // Different user ID from the user making the request
  };

  // Mocking the findById method to return the task to be updated
  Task.findById.mockResolvedValue(taskToUpdate);

  // Mocking the findById method to return the user making the request
